...
class FinalLayer(nn.Module):
    ...
    """
    def __init__(self, hidden_size, per_patch_output_size):
        super().__init__()
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        # this is required by torch's conv3D
        B, C, T, H, W = x.shape
        # _assert(H == self.img_size[0], f"Input image height ({H}) doesn't match model ({self.img_size[0]}).")
        # _assert(W == self.img_size[1], f"Input image width ({W}) doesn't match model ({self.img_size[1]}).")
        x = self.proj(x)
        if self.flatten:
            # x is BCTHW -> flatten to BC(THW) 
            x = x.flatten(2).transpose(1, 2)  # BCTHW -> B(THW)C
        x = self.norm(x)
        return x
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.skip_linear = nn.ModuleList([
            nn.Linear(2 * hidden_size, hidden_size) for _ in range(depth // 2)
        ])
    ...
    def initialize_weights(self):
        ...
        # we need to use 3d sin-cos embedding here
        pos_embed = pos_multi_embedding.get_3d_sincos_pos_embed(self.hidden_size, self.x_embedder.grid_size, cls_token=False, extra_tokens=0)
        ...
        assert self.y_embedder is None
        ...
        # for skip linear layer, we use random initialization
        for skip_linear in self.skip_linear:
            nn.init.trunc_normal_(skip_linear.weight, std=.02)
    ...
    def unpatchify3D(self, x):
        """
        x: (N, T, per_patch_output_size * out_channels)
        imgs: (N, H, W, C)
        """
        # the secret here is to make the transformer an identity function, so we need to be careful of the order here...
    ...
    def forward(self, x, t, y):
        ...
        for i in range(self.depth // 2 + 1):
            ...
            if i != self.depth // 2:
                result.append(x)  
        assert len(result) == self.depth // 2
        for i in range(self.depth // 2):
            block = self.blocks[i + (self.depth // 2) + 1]
            x = torch.cat((x, result[-(i + 1)]), dim = -1) # (N, T, 2D)  
            x = self.skip_linear[i](x)
            x = block(x, c)

        x = self.final_layer(x, c)                # (N, T, per_patch_output_size * out_channels)
        x = self.unpatchify3D(x)                   # (N, out_channels, H, W)
        return x
